...
import { documentationMetadata, toolDocumentationMap, ToolDocumentation } from './documentation-metadata.js';
 ...
 */
export function findDocumentationForTool(toolName: string): string[] {
 ...
 */
export function getDocumentationDetails(path: string): ToolDocumentation | undefined {
 ...
 */
export function findDocumentationForTools(toolNames: string[]): Record<string, string[]> {
 ...
 */
export function findRelatedTools(toolName: string): string[] {
  const docs = findDocumentationForTool(toolName);
  if (!docs.length) return [];
  
  const relatedTools = new Set<string>();
 ...
 */
export function findCommonDocumentation(toolNames: string[]): string[] {
  ...
  for (let i = 1; i < toolNames.length; i++) {
    ...
    commonDocs = new Set(
      Array.from(commonDocs).filter(doc => toolDocs.has(doc))
    );
  }
